...
export const bookingService = {
  ...
  async getByMonth(companyId: string, year: number, month: number): Promise<BookingWithCreator[]> {
    ...
    const { data, error } = await supabase
      .from('bookings')
      .select('*, creator:profiles!bookings_created_by_fkey(id, full_name, email), updater:profiles!bookings_updated_by_fkey(id, full_name, email), resource:resources(id, name, metadata)')
      .eq('company_id', companyId)
      .lt('starts_at', end)
      .gt('ends_at', start)
